...
public final class DifferentialFlamePalette implements IDataPalette {
    ...
    private static String generateGreen(int i) {
        return "GREEN" + i; //$NON-NLS-1$
    }
    ...
    static {
     ...
     // Add Green tenses for Shorter duration
        for (i = MIN_HUE; i <= MAX_HUE; i++) {
            j = (i-50)>0 ? i-50:0;
            String greenKey = generateGreen(i);
            builder.put(greenKey, new OutputElementStyle(null, ImmutableMap.of(
                    StyleProperties.STYLE_NAME, greenKey,
                    StyleProperties.BACKGROUND_COLOR, X11ColorUtils.toHexColor(j, 255, i),
                    StyleProperties.OPACITY, 1,
                    StyleProperties.BORDER_STYLE, StyleProperties.BorderStyle.SOLID)));
        }
    ...
    public OutputElementStyle getStyleFor(Object object) {
        if (object instanceof DifferentialWeightedTree) {
            ...
            if (difference < 0) {
                return STYLE_MAP.computeIfAbsent(generateGreen((int) (MAX_HUE + Math.floor(difference * step))), OutputElementStyle::new);
            }
