...
public class DifferentialFlameGraphView extends TmfView {
     ...
     * ID of the view
     */
    public static final String ID = DifferentialFlameGraphView.class.getPackage().getName() + ".diffflamegraphView1"; //$NON-NLS-1$
...
//    private static final @NonNull String SYMBOL_MAPPING_ICON_PATH = "icons/obj16/binaries_obj.gif"; //$NON-NLS-1$
    ...
    private void buildEntryList(@NonNull ITmfTrace trace, @NonNull ITmfTrace parentTrace, @NonNull Map<String, Object> additionalParams, @NonNull IProgressMonitor monitor) {
        ...
        if (trace != null) {
            ...
            fdataProviderGroup = new FlameGraphDataProvider(trace, fAnalysisModule.getDiffProvider(), FlameGraphDataProvider.ID + ':' + DifferentialCallGraphAnalysis.ID);
        }

        BaseDataProviderTimeGraphPresentationProvider presentationProvider = fPresentationProvider;
        if (presentationProvider != null) {
            presentationProvider.addProvider(fdataProviderGroup, getTooltipResolver(fdataProviderGroup));
        }
        ...
        while (!complete && !monitor.isCanceled()) {
            ...
            TmfModelResponse<TmfTreeModel<@NonNull TimeGraphEntryModel>> responseGroupA = fdataProviderGroup.fetchTree(parameters, monitor);

            if (responseGroupA.getStatus() == ITmfResponse.Status.FAILED) {
                System.out.println(responseGroupA.getStatus());
                Activator.getDefault().logError(getClass().getSimpleName() + " Data Provider failed: " + responseGroupA.getStatusMessage()); //$NON-NLS-1$
                return;
            ...
            complete = true;
            TmfTreeModel<@NonNull TimeGraphEntryModel> GroupAmodel = responseGroupA.getModel();
            long endTimeN = Long.MIN_VALUE;
            List<TimeGraphEntry> orphaned = new ArrayList<>();
            Map<Long, TimeGraphEntry> entriesN = new HashMap<>();

            if ((GroupAmodel != null)) {
                synchronized (fEntries) {
                    entriesN = fEntries.computeIfAbsent(fdataProviderGroup, dp -> new HashMap<>());
                    /*
                    ...
                    for (TimeGraphEntryModel entry : GroupAmodel.getEntries()) {
                        TimeGraphEntry uiEntry = entriesN.get(entry.getId());
                        if (entry.getParentId() != -1) {
                            if (uiEntry == null) {
                                uiEntry = new TimeGraphEntry(entry);
                                TimeGraphEntry parent = entriesN.get(entry.getParentId());
                                if (parent != null) {
                                    parent.addChild(uiEntry);
                                } else {
                                    orphaned.add(uiEntry);
                                }
                                entriesN.put(entry.getId(), uiEntry);
                            } else {
                            ...
                            if (uiEntry != null) {
                                ...
                                // trace entries
                                uiEntry = new ParentEntry(newEntry, fdataProviderGroup);
                                entriesN.put(entry.getId(), uiEntry);
                    ...
                    for (TimeGraphEntry orphanedEntry : orphaned) {
                        TimeGraphEntry parent = entriesN.get(orphanedEntry.getEntryModel().getParentId());
                        if (parent != null) {
                ...
                zoomEntries(ImmutableList.copyOf(entriesN.values()), start, endTimeN, resolutionN, monitor);
    ...
    private void zoomEntries(@NonNull Iterable<@NonNull TimeGraphEntry> normalEntries, long zoomStartTime, long zoomEndTime, long resolution, @NonNull IProgressMonitor monitor) {
        if ((resolution < 0)) {
        ...
        if (model == null) {
            System.out.print("The base execution's model is null");
            return;
        }
...
//        fConfigureSymbolsAction.setImageDescriptor(Activator.getDefault().getImageDescripterFromPath(SYMBOL_MAPPING_ICON_PATH));
    ...
    public void analysisStart(TmfStartAnalysisSignal signal) {
        ...
        IAnalysisModule module = signal.getAnalysisModule(); // It is not
                                                             // possible to link
                                                             // the module ID to
                                                             // the data
                                                             // provider, so //
                                                             // just rebuild
        // if the started module is a weighted tree provider // FIXME This may
    ...
    public void setDiffCallGrapProvider(DifferentialCallGraphAnalysis diffCallGraphProvider) {
        fAnalysisModule = diffCallGraphProvider;
    }
