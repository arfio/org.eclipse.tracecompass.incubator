...
import org.eclipse.tracecompass.incubator.concurrentcallstack.core.SpanCallGraphAnalysis;
 ...
 *
 */
public class SpanAggregatedCalledFunction extends AggregatedCallSite {
     ...
     */
    public SpanAggregatedCalledFunction(ICallStackSymbol symbol) {
        super(symbol, 0);
     ...
     *            Object to copy
     */
    public SpanAggregatedCalledFunction(SpanAggregatedCalledFunction toCopy) {
        super(toCopy);
        fStatistics = new AggregatedCalledFunctionStatistics();
    ...
    @Override
    public SpanAggregatedCalledFunction copyOf() {
        return new SpanAggregatedCalledFunction(this);
    }

    @Override
    protected void mergeData(@NonNull WeightedTree<ICallStackSymbol> other) {
        if (!(other instanceof SpanAggregatedCalledFunction)) {
            return;
        }
        SpanAggregatedCalledFunction otherFct = (SpanAggregatedCalledFunction) other;
    ...
    private void mergeProcessStatuses(SpanAggregatedCalledFunction other) {
     ...
     */
    public synchronized void addChild(SpanAbstractCalledFunction child, SpanAggregatedCalledFunction aggregatedChild) {
     ...
     */
    public synchronized void addFunctionCall(SpanAbstractCalledFunction function) {
    ...
    public @Nullable IStatistics<?> getStatistics(int metricIndex) {
        ...
        if (metricIndex == SpanCallGraphAnalysis.SELF_TIME_METRIC_INDEX) {
            return getFunctionStatistics().getSelfTimeStatistics();
        }
        if (metricIndex == SpanCallGraphAnalysis.CPU_TIME_METRIC_INDEX) {
    ...
    protected void mergeDataCg(@NonNull WeightedTree<ICallStackSymbol> other) {
        if (!(other instanceof SpanAggregatedCalledFunction)) {
            return;
        }
        SpanAggregatedCalledFunction otherFct = (SpanAggregatedCalledFunction) other;
        avgDuration(otherFct.getDuration());
        avgSelfTime(otherFct.getSelfTime());
        avgCpuTime(otherFct.getCpuTime());
        getFunctionStatistics().merge(otherFct.getFunctionStatistics(), true);
        mergeProcessStatuses(otherFct);
    }

    /**
     * Modify the function's duration for merging call graphs
     *
     * @param duration
     *            The amount to get avgerage the duration by
     */
    private void avgDuration(long duration) {
        fDuration = (fDuration + duration) / 2;
    }

    /**
     * Modify the self time for merging call graphs
     *
     * @param selfTime
     *            The amount of self time to get average
     */
    private void avgSelfTime(long selfTime) {
        fSelfTime = (fSelfTime + selfTime) / 2;
    }

    /**
     * Modify the Cpu time for merging call graphs
     *
     * @param cpuTime
     *            The amount of cpu time to get average
     */
    private void avgCpuTime(long cpuTime) {
        fCpuTime = (fCpuTime + cpuTime) / 2;
    }

    /*
     * public WeightedTree<ICallStackSymbol> mergCg(@NonNull
     * WeightedTree<ICallStackSymbol> other){ if
     * (!other.getObject().equals(getObject())) { throw new
     * IllegalArgumentException("Trying to merge two callGraph with different symbols"
     * ); //$NON-NLS-1$ } mergeDataCg(other); mergeChildrenCg(other);
     *
     * } }
     *
     *
     * private void mergeChildrenCg(WeightedTree<ICallStackSymbol> other) { for
     * (WeightedTree<ICallStackSymbol> otherChildSite : other.getChildren()) {
     *
     * WeightedTree<ICallStackSymbol> childSite = find( if (childSite == null) {
     * addChild(null, null) fChildren.put(childObject, otherChildSite.copyOf());
     * } else { // combine children childSite.mergeCg(otherChildSite); } } }
     */
        ...
        for(WeightedTree<ICallStackSymbol> child:getChildren()) {
            for(WeightedTree<ICallStackSymbol> otherchild:other.getChildren()) {
                ...
                if (child.getWeight()!= otherchild.getWeight()) {
                    ((SpanAggregatedCalledFunction) child).tuneValues(otherchild);
                }
