...
//
///**
// * If the elements in the callstack are grouped by process ID, this class should be used
// *
...
//        return ImmutableList.of(DEFAULT_PROCESSES_PATTERN);
//    }
//
//    @Override
//    protected void InstrumentedCallStackAnalysis() {
//        super();
...
import org.eclipse.tracecompass.incubator.concurrentstatesystem.core.TmfConcurrentStateSystemAnalysisModule;
...
/**
 * The base classes for analysis who want to populate the CallStack state system
 * for concurrent executions.
 *
 * @author Fateme Farji Daneshgar
 *
 */
public abstract class SpanCallStackAnalysis extends TmfConcurrentStateSystemAnalysisModule implements IFlameChartProvider, ICallGraphProvider2 {
    ...
    private @Nullable SpanCallStackSeries fCallStacks;
    ...
    @Override
    public synchronized @Nullable SpanCallStackSeries getCallStackSeries() {
        SpanCallStackSeries callstacks = fCallStacks;
        if (callstacks == null) {
            ...
            callstacks = new SpanCallStackSeries(ss, getPatterns(), 0, "", getCallStackHostResolver(trace), null); //$NON-NLS-1$
    ...
    public @Nullable ISegmentStore<ISegment> getSegmentStore() {
        SpanCallStackSeries series = getCallStackSeries();
        if (series == null) {
            return null;
        }
        return series;
