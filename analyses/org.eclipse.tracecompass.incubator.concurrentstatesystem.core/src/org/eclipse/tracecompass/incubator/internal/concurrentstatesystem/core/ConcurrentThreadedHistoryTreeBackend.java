 ...
 *
 */
public final class ConcurrentThreadedHistoryTreeBackend extends ConcurrentHistoryTreeBackend
        implements Runnable {

    private static final @NonNull Logger LOGGER = TraceCompassLog.getLogger(ConcurrentThreadedHistoryTreeBackend.class);
     ...
     */
    public ConcurrentThreadedHistoryTreeBackend(@NonNull String ssid,
            File newStateFile,
            int providerVersion,
            long startTime,
            int queueSize,
    ...
    public Iterable<@NonNull ITmfStateInterval> query2D(IntegerRangeCondition quarks, TimeRangeCondition times)
        ...
        try (FlowScopeLog log = new FlowScopeLogBuilder(LOGGER, Level.FINEST, "ThreadedHistoryTreeBackend:query2D", //$NON-NLS-1$
