 ...
 *
 */
public class ConcurrentTransientState {

    private List<Integer> fAttributeQuark;
    private List<Integer> fstackDepth;
     ...
     */
    public ConcurrentTransientState(IStateHistoryBackend backend) {
        fBackend = backend;
        fIsActive = true;
        ...
        fAttributeQuark = new ArrayList<>();
        fstackDepth = new ArrayList<>();
        ...
        try {
            ...
            if (fOngoingStateStartTimes.get(quark) < eventTime) {
                ...
                fBackend.insertPastState(fOngoingStateStartTimes.get(quark),
                        ...
                        fAttributeQuark.get(quark), /* attribute quark */
    ...
    public Iterable<ITmfStateInterval> query2D(Collection<Integer> quarks, TimeRangeCondition timeCondition) {
        fRWLock.readLock().lock();
        try (FlowScopeLog log = new FlowScopeLogBuilder(LOGGER, Level.FINEST, "TransientState:query2D", //$NON-NLS-1$
                "ssid", fBackend.getSSID(), //$NON-NLS-1$
                "quarks", quarks, //$NON-NLS-1$
                "time", timeCondition).build()) { //$NON-NLS-1$
            if (!fIsActive) {
            ...
            for (int i = 0; i < fOngoingStateInfo.size(); i++) {
                ...
                try {
                    fBackend.insertPastState(fOngoingStateStartTimes.get(i),
                            endTime, /* End Time */
                            fAttributeQuark.get(i), /* attribute quark */
    ...
    public void closeTransientState2(long endTime) {
        if (!this.fIsActive) {
            return;
        }
        ...
        fRWLock.writeLock().lock();
        try {
            for (int i = 1; i < fOngoingStateInfo.size(); i++) {
                if (fOngoingStateStartTimes.get(i) > endTime) {
                    /*
                     * Handle the cases where trace end > timestamp of last
                     * state change. This can happen when inserting "future"
    ...
    public void SetAttributequark(int tsQuark, int AttribyteQuark) {
        fAttributeQuark.set(tsQuark, AttribyteQuark);
    }
     ...
     * attribute tree, namely when we add sub-path attributes.
     */
    public void addEmptyEntry(int AttributeQuark) {
        addEmptyEntry();
        fAttributeQuark.add(AttributeQuark);
        if (fstackDepth.size() > 2) {
            fstackDepth.add(-1);
        } else {
            fstackDepth.add(null);

        }

    }

    public int TuneStackDepth(String functionEntryParent) {
        // int diff =0;
        int indx = getOngoingStateValues().size() - 1;
        while (getOngoingStateValue(indx) == null) {
            indx--;
        }
        if (indx < 2) {
            throw new NullPointerException("There is no parent for Span=:" + functionEntryParent);
        }
        String spanName = ((SpanCustomValue) (getOngoingStateValue(indx))).getSpanId();

        while (!spanName.equals(functionEntryParent)) {
            // diff++;
            indx--;
            if (indx < 2) {
                throw new NullPointerException("There is no parent for Span=:" + functionEntryParent);
            ...
            spanName = ((SpanCustomValue) (getOngoingStateValue(indx))).getSpanId();
        }

        return fstackDepth.get(indx);
    }

    public void modifyStackDepth(int attributeQuark, int stackDepth) {
        fstackDepth.set(attributeQuark, stackDepth);

    }

    public int getCallStack() {
        int indx = fstackDepth.size() - 1;
        if (fstackDepth.get(indx) == null) {
            return 0;
        }
        while (fstackDepth.get(indx) == -1) {
            indx--;
            if (fstackDepth.get(indx) == null) {
                return 0;
            }
    ...
    public int getAttributeQuarck(int tsQuark) {
        return fAttributeQuark.get(tsQuark);
    }

    public int getStackDepth(int tsQuark) {
        return fstackDepth.get(tsQuark);
    }
